...
whole point of them. A baseline may only cut edges
...
(source 718: hop0 `p` gives sigma=48, hop0∪hop1 `p` gives sigma=642).
...
. The one exception is `random`, whose *scores* come from a seeded
...
opponent and belongs in the comparison eventually. Deferred, not
forgotten.
"""
...
def run_greedy(ctx, heuristic: str, k: int, evaluator, seed: int = None) -> dict:
    ...
    CSV row per method without special-casing (one row
    ...
    k > out(s) is the trivial case: the cut is all of
    ...
    # Tie frequency is reported as a structural fact about each
